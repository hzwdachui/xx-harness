...
| 数据层 | Adapter 模式，MVP 用 SQLite，后续可选 PostgreSQL/MySQL |
| Agent 引擎 | Claude Code |
| 工作空间 | git worktree（在 xx-harness/workspace/ 下隔离） |
| Skills | 任意 Claude Code skills / plugins |
...
function run_task(task_id, trigger_source):
    ...
    for each ready_node in dag.ready_nodes():  # 无未完成依赖的节点
        ...
        for each node in batch:
            ctx = assemble_context(node, task_id)  # 通过数据层查上下文
            result = start_claude_code(node.agent, ctx, worktree)
            db.save(result)
...
每个 Claude Code 进程启动时，编排引擎通过数据层组装三层上下文：
